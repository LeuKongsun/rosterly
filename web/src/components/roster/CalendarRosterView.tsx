...
export function CalendarRosterView({
  ...
  if (isMobile) {
    ...
    // Always show compact month grid on mobile for all timeframe views (day/week/month)
    const visibleDates = (() => {
      if (timeframeView === "month") {
        ...
        // day and week: show the days as-is
        return days as (string | null)[];
      }
    })();
    ...
    return (
      <div className="flex flex-col gap-3 w-full animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        <div className="rounded-2xl border border-zinc-200/50 bg-white/60 p-2 shadow-sm dark:border-white/5 dark:bg-zinc-950/20 backdrop-blur-xl w-full overflow-hidden">
          {/* Weekday headers */}
            ...
            {visibleDates.map((day, index) => {
              ...
              return (
                <div
                  ...
                  className={`flex flex-col items-stretch rounded-lg p-1 min-h-[60px] border transition-all duration-200 cursor-pointer ${
                    isSelected
                      ? "bg-indigo-50/60 border-indigo-400 dark:bg-indigo-950/20 dark:border-indigo-400/60 ring-1 ring-indigo-400/40"
                      : isToday
                ...
                >
                  <span className={`text-[0.68rem] font-extrabold mb-0.5 ${
                    isSelected
        ...
        {activeDay && (
          <div className="rounded-2xl border border-zinc-200/50 bg-white/60 p-4 shadow-sm dark:border-white/5 dark:bg-zinc-950/20 backdrop-blur-xl flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-3 duration-300">
